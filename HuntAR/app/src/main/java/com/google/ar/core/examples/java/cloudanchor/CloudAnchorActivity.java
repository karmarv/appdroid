...
import com.google.ar.core.examples.java.common.helpers.GPSTracker;
...
public class CloudAnchorActivity extends AppCompatActivity implements GLSurfaceView.Renderer {
  ...
  private final class RoomCodeAndCloudAnchorIdListener
    ...
    /**
     * Share the location information using firebase
     */
    private void checkAndMaybeShare() {
      if (roomCode == null || cloudAnchorId == null) {
        return;
      }
      String locationMsg = getLocationInformation();
      String storeMessage = "Treasure has been added at time "+ new Timestamp(System.currentTimeMillis())+", "+locationMsg;
      firebaseManager.storeAnchorIdInRoom(roomCode, cloudAnchorId, storeMessage);
      snackbarHelper.showMessageWithDismiss(
          CloudAnchorActivity.this, getString(R.string.snackbar_cloud_id_shared));
      Log.i("AnchorActivity","Msg: "+storeMessage);
    }

    /**
     * Get geo location from GPS tracker
     *
     * @return
     */
    private String getLocationInformation(){
        // Request permission for this activity
        AppPermissionHelper.requestLocationPermission(CloudAnchorActivity.this);
        // Create class object
        GPSTracker gps = new GPSTracker(CloudAnchorActivity.this);
        String msg = "";
        // Check if GPS enabled
        if(gps.canGetLocation()) {
            double latitude = gps.getLatitude();
            double longitude = gps.getLongitude();
            msg = "Location, Lat:" + latitude + ", Long:" + longitude;
            // \n is for new line
